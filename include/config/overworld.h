...
// For defining the clock in overworld.h to adjust to custom RTC
...
#define OW_USE_FAKE_RTC                 TRUE       // When TRUE, seconds on the in-game clock will only advance once every 60 playTimeVBlanks (every 60 frames).
                                                   // I'm gonna assume when you're PRing this branch it's already on so I've set it as such.
#define OW_ALTERED_TIME_RATIO           RTC_CUSTOM // In GEN_8_PLA, the time in game moves forward 60 seconds for every second in the RTC. In GEN_9, it is 20 seconds. You can change its ratio in fake_rtc.c
                                                   //This has no effect if OW_USE_FAKE_RTC is FALSE. 
